...
struct imx6uirq_dev{
    ...
    struct fasync_strcut *async_queue; //异步相关的结构体
};
...
void timer_function(unsigned long arg)
{
    ...
    if(atomic_read(&dev->releasekey)){  //一次完整的按键
        if(dev->async_queue)
            kill_fasync(&dev->async_queue,SIGIO,POLL_IN);   //给应用程序发信号
    }
...
static ssize_t imx6uirq_read(struct file *filp,char __user *buf,size_t cnt ,loff_t *offt)
{
    ...
    if(atomic_read(&dev->releasekey) == 0){
        //没有按键按下
        add_wait_queue(&dev->r_wait,&wait); //添加队列头
