...
- 执行流程
  ...
  1. **软中断触发**：当`TASKLET_SOFTIRQ`或`HI_SOFTIRQ`被触发时，内核调用对应的软中断处理函数。
  2. **链表遍历**：软中断处理函数遍历`tasklet_vec`或`tasklet_hi_vec`链表，执行每个 tasklet 的处理函数。
  3. **状态检查**：处理前检查`state`和`count`，确保 tasklet 未在其他 CPU 上运行且未被禁用。
...
// 向最小堆中插入元素
void insert(Node *heap, int *heap_size, Node node) {
    heap[(*heap_size)++] = node;
    int i = *heap_size - 1;
...
int minTimeToReach(int **moveTime, int moveTimeSize, int *moveTimeColSize) {
    ...
    int heap_size = 0;
    Node start = {0, 0, 0, 2};
    insert(heap, &heap_size, start);

    int dx[] = {-1, 1, 0, 0};
    int dy[] = {0, 0, -1, 1};

    while (!isEmpty(heap_size)) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            int newY = y + dy[j];
            if (newX >= 0 && newX < n && newY >= 0 && newY < m &&!visited[newX][newY]) {
                int nextTime = (time > moveTime[newX][newY]) ? time + set : moveTime[newX][newY] + set;
                Node next = {newX, newY, nextTime, set};
                insert(heap, &heap_size, next);
            }
        }
    }
    return -1;
...
变种题，第一步时间是1，第二步时间是2，第三步时间是3。前之前的代码上将增加的时间进行维护更改是ok的，但是测试项存在大量数据，前面使用qsort排序，导致超时，但是原理一直，要找到队列里的最小的时间节点，所以将数据的存储抽象成二叉树（小根堆），这样插入、取出和排序的时间降低，时间就不会超时
...
long long minSum(int* nums1, int nums1Size, int* nums2, int nums2Size) {
    int f_size = (nums1Size > nums2Size ? nums2Size: nums1Size);        //循环的层数
    int l_size = (f_size == nums2Size? nums1Size : nums2Size);
    int idx = (l_size == nums1Size? 0: 1);
    int *last = (idx == 0? nums1: nums2);
    long long sum[2] = {0,0};
    int zero_num[2] = {0,0};    
    ...
    for(int i  = f_size;i < l_size; i++)
    {
        sum[idx] += last[i];
        if(!last[i])
            zero_num[idx]++;
    }
    printf("%lld %d %lld %d\n",sum[0],zero_num[0],sum[1],zero_num[1]);
    if((zero_num[0] == 0 && zero_num[1] == 0 && sum[0] != sum[1]) ||
        (zero_num[0] == 0 && zero_num[1] != 0 && sum[0] < (sum[1] + zero_num[1])) || 
        (zero_num[0] != 0 && zero_num[1] == 0 && (sum[0] + zero_num[0]) > sum[1] ) )
        return -1;
    
    return fmax(sum[0] + zero_num[0],sum[1] + zero_num[1]);
}
````
